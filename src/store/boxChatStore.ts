...
export const boxChatStore = create<State>((set) => ({
  ...
  updateBoxChat: (idBox: string, type: string, roomMsg: CardRoom | undefined) =>
    set((state) => {
      const existingBox = state.listBoxChat.find(
        (box) => box.idBox === idBox && box.type === type
      );
      ...
      if (type === "person") {
        const exitsBox = state.listBoxChat.find(
          (box) => box.roomMsg?.user._id === idBox
        );
        if (exitsBox) {
          return { listBoxChat: [...state.listBoxChat] };
        }
      ...
      if (state.listBoxChat.length === 3) {
        return {
          listBoxChat: [
            ...state.listBoxChat.slice(1, 3),
            { idBox: idBox, type: type, roomMsg: roomMsg },
          ],
        };
      }
      return {
