...
const CommentCard = React.memo((props: Props) => {
  ...
  const [newRepComments, setNewRepComments] = useState<
    Omit<CommentProps, "repLength">[]
  >([]);
  ...
  const handleLoadMoreRepComment = async (
    ...
    try {
      socket.emit(
        "load-repcomment-more",
        comment._id,
        page,
        newRepComments.length
      );
      setPage(page + 1);
      socket
        ...
        .on(
          "load-repcomment-more",
          (newListComments: Omit<CommentProps, "repLength">[]) => {
  ...
  socket
    .off("delete-repcomment")
    .on("delete-repcomment", handleSocketDeleteRepComment);
  ...
  socket
    .off("change-rep-comment")
    .on("change-rep-comment", handleSocketChangeComment);
  ...
  socket
    .off("delete-rep-comment")
    .on("delete-rep-comment", handleSocketDeleteOneRepComment);
        ...
        <div className="flex gap-2 w-full">
          ...
          {isChange ? (
            <form
              ...
              className="flex flex-col w-full bg-gray-200 rounded-[8px] z-50"
            >
              <textarea
