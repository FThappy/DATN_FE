            ...
            {item.wallImg.length > 0 && (
              <Carousel>
                <CarouselContent className="w-full desktop:h-[35rem] laptop:h-[30rem] ml-0 pl-0 relative">
                  ...
                  <Link href={`/event/${item?._id}`} className="absolute flex gap-2 bg-black/40 bottom-0 w-full rounded-b-[8px] px-2 h-[5rem] items-center cursor-pointer 
                  hover:bg-black/75
                  ">
                    ...
                    <div className="flex flex-col">
                      <p className="font-bold	text-red text-[1rem]">
                        {item?.timeStart && item?.timeEnd
                          ? timeFormatCustom(new Date(item.timeStart)) +
                            "-" +
