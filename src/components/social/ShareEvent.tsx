...
const ShareEvent = (props: Props) => {
  ...
  useEffect(() => {
    const getItem = async () => {
      setPending(true);
      try {
        const res = await getEventById(itemId);
        if (res.code === 0) {
          setPending(false);
            ...
            {item.wallImg.length > 0 && (
              <Carousel>
                <CarouselContent className='w-full desktop:h-[35rem] laptop:h-[30rem] ml-0 pl-0 relative'>
                  ...
                  '
                  >
                    <div className='shadow-beautiful bg-white w-[4rem] h-[4rem] rounded-[12px]'>
                      <div className='bg-red h-[1rem] rounded-t-[12px]'></div>
                      <div className='flex justify-center items-center w-full h-[3rem] text-[1.5rem] font-bold  '>
                        {item?.timeStart && new Date(item.timeStart).getDate()}
                      </div>
                    </div>
                    <div className='flex flex-col'>
                      <p className='font-bold	text-red text-[1rem]'>
                        {item?.timeStart && item?.timeEnd
                          ? timeFormatCustom(new Date(item.timeStart)) + '-' + timeFormatCustom(new Date(item.timeEnd))
                          : ''}
                      </p>
                      <p className='font-medium text-white	 text-[1.5rem]'>{item?.eventName}</p>
                    </div>
                  </Link>
                </CarouselContent>
                {item.wallImg.length > 1 ? (
                  <>
              ...
              <div className='flex flex-col '>
                ...
                <div className='flex gap-2'>
                  <p className=' w-[200px] text-gray-400 cursor-pointer'>
                    {differenceInHours(new Date(), new Date(item?.createdAt)) <= 0
                      ? 'Vài phút trước'
                      : differenceInHours(new Date(), new Date(item?.createdAt)) >= 24
                        ? differenceInDays(new Date(), new Date(item?.createdAt)) + ' ngày trước'
                        : differenceInHours(new Date(), new Date(item?.createdAt)) + 'h'}
                  </p>
                </div>
              </div>
            </div>
