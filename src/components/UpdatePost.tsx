...
const UpdatePost = (props: Props) => {
  ...
  const removeFile = (index: number) => {
    if (typeof files[index] === 'string') {
      setRemoveFiles((prev: string[]): string[] => [...prev, files[index] as string]);
    }
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        {!active ? (
              ...
              <div className='w-full h-[60px] cursor-pointer flex items-center gap-2 p-2  mb-2'>
                ...
                {files.length > 0 && (
                  <div className='flex gap-2 ml-[15rem]'>
                    ...
                    >
                      <RiPencilFill size={24} /> <p className='font-bold'>Chỉnh sửa</p>
                    </button>
                    <label htmlFor='file'>
          ...
          h-[50px]  cursor-pointer'
              ...
              <button
                type='submit'
                className='h-[40px] w-[95%] mb-2 mt-2 bg-greenPrimary rounded flex justify-center items-center font-bold text-white text-[1.2rem] cursor-pointer'
                disabled={
                ...
                style={{
                  backgroundColor: files.length <= 0 && !documention ? '#F84D42' : '#20b86d'
                }}
