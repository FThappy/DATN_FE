...
const UpdateProject = (props: Props) => {
  ...
  const [tmpImg, setTmpImg] = useState<any>(project?.content?.content?.filter(url => url.type === 'image'));
  ...
  const handleSubmit = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (!user) {
      return toastifyUtils('info', 'Vui lòng đăng nhập để thực hiện chức năng này');
    }
    setPending(true);
    try {
      if (
      ...
      if (imageContentRemoved.length > 0) {
        formData.append('imageContentRemove', JSON.stringify(imageContentRemoved));
      }
      ...
      setNewFiles([]);
      setFileRemove([]);
      setTmpImg(res.project?.content?.content?.filter(url => url.type === 'image'));
      setImageContent([]);
  ...
  useEffect(() => {
    tmpImg.map((item, index) => {
      if (!contentJSON?.content?.filter(url => url.type === 'image').find(url => url.attrs.src === item.attrs.src)) {
    ...
    <div className='flex flex-col gap-1 items-center bg-[#f1eff4d1]  '>
      ...
      <form className='flex flex-col gap-1 items-center'>
        ...
        <button
          ...
          className={`h-[40px] w-[98%] mr-4 mb-2 mt-2 bg-greenPrimary rounded flex justify-center 
