...
const ReportModal = (props: Props) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (checked) {
      ...
      setReason(prevReasons => prevReasons.filter(reason => reason !== value));
    }
  };
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        ...
        >
          ...
          <p className='text-gray-400 px-4'>
            Nếu bạn nhận thấy ai đó đang gây nguy hiểm, đừng chần chừ mà hãy báo cáo với chúng tôi.
          </p>
          <form className='flex flex-col items-center px-4'>
            ...
            <button
              type='submit'
              className='h-[40px] w-[100%] mb-4 bg-greenPrimary rounded flex justify-center items-center font-bold text-white text-[1.2rem]'
              disabled={(reason.length <= 0 && !detail) || pending}
              style={{
                backgroundColor: reason.length <= 0 && !detail ? '#F84D42' : '#20b86d'
              }}
