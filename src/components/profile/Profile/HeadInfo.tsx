...
const HeadInfo = (props: Props) => {
  ...
  const handleChangeImage = async (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    <>
      ...
      <div className='w-[80%] h-[12rem] absolute bottom-[-12rem] right-0 flex gap-1 justify-between '>
        <div className='flex flex-col gap-1 mt-2'>
          <p className='font-bold text-[2.5rem]'>{user.displayname ? user.displayname : user.username}</p>
          <p className='font-bold text-[1.5rem] text-gray-400'>{user.displayname && user.username}</p>
        </div>
        <div className='flex gap-2 justify-center items-center mr-4'>
          {fileImage && (
            <button
              className='flex gap-2 justify-center items-center p-2 bg-green w-[10rem] rounded-[6px] cursor-pointer h-[2.5rem] hover:bg-greenPrimary/50
