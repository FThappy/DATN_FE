      ...
      <Link href={`/event/${event._id}`} className='w-full h-[23rem]'>
        ...
        <div className='flex flex-col px-4 mt-1 '>
          <p className='font-medium	 text-[0.9rem] text-red'>
            {event?.timeStart && event?.timeEnd && new Date() > new Date(event.timeStart)
          ...
          <p className='font-medium	text-gray-400 text-[0.8rem]'>{totalJoin} người sẽ tham gia</p>
          <button className=' flex justify-center items-center bg-greenPrimary p-1 rounded-[8px] hover:bg-greenPrimary/75 mt-1 font-bold text-white text-[1.2rem]'>
            Xem sự kiện
          </button>
        </div>
      </Link>
