  ...
  return (
    <div className='flex shadow-beautiful bg-white p-2 rounded-[8px] px-4 gap-2'>
      ...
      <div className='w-1/2 border rounded-[8px] h-[23rem] flex flex-col items-center p-2 cursor-pointer hover:bg-gray-200'>
        <img src={user?.img} alt='image' className='  w-[14rem] h-[14rem] rounded-full object-cover' loading='lazy' />
        <p className='font-bold text-[1.2rem] mt-2'>{user?.displayname ? user.displayname : user?.username}</p>
