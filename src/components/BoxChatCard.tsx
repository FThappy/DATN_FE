...
const BoxChatCard = (props: Props) => {
  ...
  useEffect(() => {
    const handleNewRoom = (newRoom: Room, newMessage: MessageProp, userId: string) => {
      if (roomId === userId) {
  ...
  const handleLoadMoreMess = async (): Promise<void> => {
    setIsLoading(true);
    try {
      const res = await getMessageForRoom(room?._id, listMessage.length > 0 ? listMessage.length : 0);
      if (res.code === 4) {
        setIsLoading(false);
        toastifyUtils('error', 'Lỗi server');
      }
        ...
        <div className='h-[5rem] p-2 w-full flex flex-wrap gap-1 bg-white relative border-t-1 border-gray-300 max-w-[100%] overflow-y-auto'>
          {fileImage.map((file, index) => (
            <div key={index} className='relative'>
              <img src={URL.createObjectURL(file)} alt='image' className='w-[5rem] h-[5rem] cursor-pointer' />
              <button
