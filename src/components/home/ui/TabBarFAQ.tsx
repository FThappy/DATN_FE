  ...
  return (
    <div className='flex flex-col gap-4'>
      ...
      {tabFAQ.map((item, index) => (
        <div key={index} className='flex flex-col  border-inherit border-b-2	p-2'>
          <div className='flex items-center mb-4'>
            <button
              className={`  ${
                inter.className
              }  border-2  rounded-full w-[3rem] h-[3rem] text-[1.5rem] flex items-center justify-center
          ${isActive === index ? 'text-red border-red' : 'text-gray-400 border-inherit'}
          `}
              onClick={() => setIsActive(index)}
            >
              {isActive === index ? '-' : '+'}
            </button>
            <p className={`${inter.className} font-bold text-slate-800 text-[2rem] ml-4 `}>{item.title}</p>
          </div>
          <p
