...
const Notification = (props: Props) => {
  ...
  const [listNewNotifications, setListNewNotifications] = useState<NotificationProps[]>([]);

  const [listNotifications, setListNotifications] = useState<NotificationProps[]>([]);
  ...
  useEffect(() => {
    const handleRemoveNotification = (notificationId: string) => {
      setListNewNotifications(prev => prev.filter(notification => notification._id !== notificationId));
      setListNotifications(prev => prev.filter(notification => notification._id !== notificationId));
      setNumberNotifications(prev => {
        if (listNewNotifications.findIndex(item => item._id === notificationId)) {
          return prev - 1;
        }
        if (listNotifications.findIndex(item => item._id === notificationId)) {
          return prev - 1;
        }
        return prev;
      });
  ...
  useEffect(() => {
    const handleUpdateNotification = (newNotification: NotificationProps) => {
      setListNewNotifications(prev =>
        prev.map(notification => (notification._id === newNotification._id ? newNotification : notification))
      );
      setListNotifications(prev =>
        prev.map(notification => (notification._id === newNotification._id ? newNotification : notification))
      );
      console.log('a');
      setNumberNotifications(prev => {
        if (listNewNotifications.findIndex(item => item._id === newNotification._id)) {
          return prev + 1;
        }
        if (listNotifications.findIndex(item => item._id === newNotification._id)) {
          return prev + 1;
        }
        return prev;
      });
  ...
  const handleChangeRead = (newNotification: NotificationProps, type: string) => {
    if (type === 'new') {
      setListNewNotifications(prev => {
        const newList = [...prev];
        const index = newList.findIndex(item => item._id === newNotification._id);
      ...
      <PopoverContent className='mr-2 w-[25rem] max-h-[calc(80vh-8rem)] min-h-[calc(80vh-8rem)] overflow-y-scroll z-[100]'>
        <div className='flex flex-col gap-2'>
          ...
          {end ? (
            ...
            <p className='text-center text-[1.5rem] my-4 text-gray-400 font-bold'>Đã hết thông báo</p>
          )}
        </div>
      </PopoverContent>
    </Popover>
