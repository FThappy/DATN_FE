...
const LoginPage = () => {
  ...
  const handleLogin = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const _password = CryptoJS.AES.encrypt(formData.get('password'), process.env.NEXT_PUBLIC_KEY_AES).toString();
    const data = {
  ...
  return (
    <div className='flex flex-col w-[60%] items-center mt-8'>
      <p className={`${dancing.className} font-bold text-red text-[5rem] mt-4 mb-2`}>Login</p>
      ...
      after:hover:left-[0px] after:hover:bottom-[0px] after:hover:z-[-20]  bg-greenPrimary	 animatie 
      `}
        >
          Đăng nhập
        </button>
      ...
      <Link href='/register' className='text-sky-600 text-center text-[1.2rem]'>
        Đăng ký
      </Link>
    </div>
  );
