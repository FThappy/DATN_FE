...
const OTPPage = () => {
  ...
  const handleReSendOTP = async (event: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
    event.preventDefault();
    try {
  ...
  return (
    <div className='flex flex-col w-[60%] items-center mt-8'>
      <p className={`${dancing.className} font-bold text-red text-[5rem] mt-4 mb-2`}>Xác thực đăng ký</p>

      <form onSubmit={handleRegister} className='gap-2 flex flex-col justify-center items-center w-full'>
      ...
      <p className='text-white mt-4'>Chúng tôi đã gửi otp đến email : {email}</p>
