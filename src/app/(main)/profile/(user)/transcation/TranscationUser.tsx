...
const TranscationUser = () => {
  ...
  const [textSearch, setTextSearch] = useState<string>(pSearch && pSearch !== 'undefined' ? pSearch : '');
  ...
  const getListTranscationSearch = async (): Promise<void> => {
    ...
    try {
      ...
      if (res.code === 0) {
        setIsLoading(false);
        setListTranscation(prevTranscation => [...prevTranscation, ...res.listTranscation]);
        setPage(page + 1);
      }
    } catch (error) {
      setIsLoading(false);
  ...
  const handleLoadMore = async () => {
    if ((pSearch && pSearch !== 'undefined') || (pDate && pDate !== 'undefined')) {
      console.log('11');
  ...
  const handlePushParams = (date: Date | undefined, textSearch: string | undefined) => {
    router.push(`/profile/transcation?qDate=${date}&&qSearch=${textSearch}`);
  };
      ...
      <div className='shadow-beautiful w-full h-screen flex flex-col bg-white rounded-[8px]'>
        ...
        <div className='h-[80%] w-full px-8 pt-1 flex flex-col items-center'>
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className='text-[1.5rem]  w-[35%]'>Số tiền đã chuyển</TableHead>
                <TableHead className='text-[1.5rem]  w-[35%]'>Tên dự án từ thiện</TableHead>
                <TableHead className='text-right text-[1.5rem] w-[20%]'>Ngày chuyển tiền</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {listTranscation.map((transcation, index) => (
                <RowTranscation index={index} transcation={transcation} key={index} />
              ))}
              {isLoading && (
                <>
          ...
          {!endTranscation ? (
            ...
            <p className='text-center text-[1.5rem] my-4 text-gray-400 font-bold'>{/* - Đã hết giao dịch - */}</p>
          )}
        </div>
      </div>
    </div>
