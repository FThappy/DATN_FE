...
const RowTranscation = (props: Props) => {
  ...
  return (
    <TableRow key={index}>
      <TableCell className='font-medium w-[150px] text-[1.2rem] flex  py-4'>{index + 1}</TableCell>
      ...
      <TableCell className='text-[1.2rem] w-[30rem] py-4'>
        <Link href={`/project/${transcation.projectId}`} className='hover:text-blue'>
          {project?.projectName ? project?.projectName : <Skeleton className='w-[100%] h-[1.4rem] rounded-[8px]' />}
        </Link>
      </TableCell>
