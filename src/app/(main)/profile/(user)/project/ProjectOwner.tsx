import { getProjectByOwner } from '@/actions/getProjectByOwner';
...
const ProjectOwner = (props: Props) => {
  ...
  const getListProject = async (): Promise<void> => {
    ...
    // console.log(page)
    try {
      const res = await getProjectByOwner(page);
      if (res.code === 4) {
        setIsLoading(false);
  ...
  const handlePushPrams = (
    ...
    setEndProject(true);
    router.push(`/profile/project?qType=${qType}&&qSearch=${qSearch}&&qSort=${qSort}&&qCity=${qCity}`);
  };
  ...
  const handleLoadMore = () => {
    if ((qSearch && qSearch !== 'undefined') || (qType && qType.length > 0) || (qSort && qSort !== '') || qCity) {
      // console.log("a");
      handleSearch();
    } else {
      // console.log("b");
