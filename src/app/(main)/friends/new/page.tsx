...
import { getNoFriend } from "@/actions/getNoFriend";
...
const Page = (props: Props) => {
  ...
  const [listFriendId, setListFriendId] = useState<ListId[]>([]);
  ...
  const getFriends = async () => {
    try {
      const res = await getNoFriend(page);
      if (res.code === 0) {
      ...
      <div className="shadow-beautiful w-full h-auto min-h-[55rem] flex flex-col bg-white rounded-[8px]">
        <div className="h-auto w-full px-8 pt-1 flex flex-col ">
          <div className="flex items-center justify-between">
            <p className="text-[1.5rem] font-bold mb-2">
              Tìm kiếm thêm bạn bè mới :
            </p>
            <form
              // onSubmit={(e) => {
              //   e.preventDefault();
            ...
            >
              ...
              <button
                type="submit"
                className="w-[3rem] h-[3rem] p-2 flex items-center justify-center"
              >
                <IoIosSearch size={32} />
              </button>
            </form>
