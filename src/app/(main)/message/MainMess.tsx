...
const MainMess = (props: Props) => {
  ...
  const handleFirstMessage = async (
    event: React.MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLTextAreaElement>
  ) => {
  ...
  const handleMessage = async (event: React.MouseEvent<HTMLButtonElement> | KeyboardEvent<HTMLTextAreaElement>) => {
    event.preventDefault();
    if (!content && fileImage.length <= 0) {
      return toastifyUtils('error', 'Không được để trống');
    }
    if (fileImage && fileImage.length > 0) {
      const nonImageFile = fileImage.find(file => !file.type.startsWith('image/'));
      if (nonImageFile) {
        return toastifyUtils('warning', 'Hiện chỉ hỗ trợ file ảnh');
      }
    }
  ...
  const handleLoadMoreMess = async (): Promise<void> => {
    setIsLoading(true);
    try {
      const res = await getMessageForRoom(active.room._id, listMessage.length > 0 ? listMessage.length : 0);
      if (res.code === 4) {
        setIsLoading(false);
        toastifyUtils('error', 'Lỗi server');
      }
  ...
  useEffect(() => {
    const handleDelete = (roomId: string, messageId: string) => {
      setListMessage(prevMessage => prevMessage.filter(item => item._id !== messageId));
    };
  ...
  useEffect(() => {
    const handleRead = (message: MessageProp) => {
      setListMessage(prevMessages =>
        prevMessages.map(item => (item._id === message._id ? { ...item, isRead: [...message.isRead] } : item))
      );
    };
  ...
  return (
    <div className='w-[56%] bg-white laptop:h-[39.5rem] desktop:h-[53rem] flex flex-col'>
        ...
        <div className='h-[10rem] p-2 w-full flex flex-wrap gap-1 bg-white relative border-t-1 border-gray-300 max-w-[100%] overflow-y-auto'>
          {fileImage.map((file, index) => (
            <div key={index} className='relative'>
              <img src={URL.createObjectURL(file)} alt='image' className='w-[10rem] h-[10rem] cursor-pointer' />
              <button
      ...
      <div className='w-full  px-1 flex gap-2 items-end z-[51] '>
        ...
        <label htmlFor='fileImage' className='rounded-full bottom-5 right-4 hover:bg-gray-100 cursor-pointer'>
        ...
        >
          <PiPaperPlaneRightFill color={content || fileImage.length ? '#1E90FF' : 'gray'} className='mr-2' size={20} />
        </button>
      </div>
    </div>
  );
