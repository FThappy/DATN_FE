    ...
    <div className='flex flex-col gap-1 items-center bg-[#f1eff4d1]  '>
          ...
          <CarouselContent className='w-[55rem] h-[25rem] ml-0 pl-0'>
            {files.map((file, index) => (
              <CarouselItem key={index} className='ml-0 pl-0 relative'>
                <img src={URL.createObjectURL(file)} alt='image' className='w-full h-full  cursor-pointer ' />
                <button
      ...
      <form className='flex flex-col gap-1 items-center'>
        ...
        <button
          ...
          style={{
            backgroundColor:
              !user || !projectName || !description || !date || !city || !type || !content ? '#F84D42' : '#20b86d'
          }}
          className={`h-[40px] w-[100%]  mb-2 mt-2 bg-greenPrimary rounded flex justify-center 
          shadow-beautiful items-center font-bold text-white text-[1.2rem] cursor-pointer
      `}
          onClick={handleSubmit}
          disabled={!user || !projectName || !description || !date || !city || !type || !content}
        >
          {pending ? (
            <>
              <p>Loading</p>
