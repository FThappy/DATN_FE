    ...
    <div className='bg-[#f1eff4d1] shadow-beautiful w-full pb-4'>
      ...
      <div className='px-20 bg-white'>
        ...
        <p className='font-bold	text-red text-[1.2rem]'>
          {event?.timeStart && event?.timeEnd
            ? timeFormatCustom(new Date(event.timeStart)) + '-' + timeFormatCustom(new Date(event.timeEnd))
            : ''}
        </p>
        ...
        <div className='flex justify-between items-center w-full  '>
          ...
          <div className='flex items-center gap-2 '>
            {user?.id || event?.userId ? (
              user?.id === event?.userId ? (
                <>
                  {event && <ModalUpdateEvent event={event} setEvent={setEvent} />}
                  {event?._id && (
                    <div>
                      <ModaleDeleteEvent userId={user.id} eventId={event._id} />
                    </div>
                  )}
                  {event && <ModalShareEventProject itemId={event._id} type='project' />}
                </>
              ) : (
                <>
                  {event && (
                    <>
                      <ReportModal postId={event._id} type='event' userId={user?.id} setOpenPopover={setOpenPopover}>
            ...
            <LikeContainer itemId={params.id} type='event' totalLike={totalLike} setTotalLike={setTotalLike} />
          </div>
        </div>
      </div>
      <div className='flex  w-full mt-8 gap-4 px-20'>
        ...
        <div className='flex flex-col shadow-beautiful bg-white rounded-[8px] w-[37%] h-auto p-2 px-4 overflow-y-scroll max-h-[32.5rem]'>
          ...
          {event && (
            <ListUserJoin eventId={params.id} ownerId={event.userId} listJoin={listJoin} setListJoin={setListJoin} />
          )}
        </div>
      </div>
    </div>
